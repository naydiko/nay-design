 ...
 * exists yet, and exposing every user would be a privacy leak), no hard
 * delete (per {@link com.naydiko.backend.domain.entity.User}, users are
 * never hard-deleted while they may own projects), and no registration/login
 * endpoints yet (authentication will be implemented separately later).
 */
@Tag(name = "Users")
@RestController
